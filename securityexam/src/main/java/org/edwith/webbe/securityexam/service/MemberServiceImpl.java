...
public class MemberServiceImpl implements MemberService{
	...
	public void addMember(Member member, boolean b) {
		...
		if(memberId < 0) throw new RuntimeException("회원가입 오류!");
		//role 부여
		String roleName = "ROLE_USER";
		MemberRole memberRole = new MemberRole(memberId,roleName);
		int result = memberRoleDao.addMemberRole(memberRole);
		
		//boolean값 true이면 관리자(?)
		if(b) {
			//관리자
			roleName = "ROLE_ADMIN";
			result = memberRoleDao.addMemberRole(memberRole);
		}
		if(result < 0) throw new RuntimeException("회원가입 오류!(권한)");
	}
	

	@Override
	public Member getMemberByEmail(String loginId) {
		Member m = memberDao.getMemberByEmail(loginId);
		return m;
	}
